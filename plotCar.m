...
%     h_rc_r = app.RollCenterheightrearmmEditField.Value;
    ...
    if (app.DrawCoGCheckBox.Value)       
        % Draw CoG
        %scatter3(app.UIAxes,x_cog, y_cog, z_cog);
        plot3(app.UIAxes,[x_cog; x_cog - 100], [0; 0], [z_cog; z_cog], 'Color', 'r');
        plot3(app.UIAxes,[x_cog; x_cog], [0; 100], [z_cog; z_cog], 'Color', 'g');
        plot3(app.UIAxes,[x_cog; x_cog], [0; 0], [z_cog; z_cog + 100], 'Color', 'b');
    end 
    
    
    %% Wheel Center axis
    if (app.DrawWheelaxisCheckBox.Value)
        plot3(app.UIAxes,[x_fa; x_fa],[-track_f/2; track_f/2],[tire_radius tire_radius])
        plot3(app.UIAxes,[x_fa+wheelbase; x_fa+wheelbase],[-track_r/2; track_r/2],[tire_radius tire_radius])
    end
    ...
    for i = 0:2:tire_width
        ...
        if (app.DrawFrontTiresCheckBox.Value)
            % FR
            plot3(app.UIAxes,x_f,ones(1,numel(x_f))*track_f/2+i/2,z, 'k')
            plot3(app.UIAxes,x_f,ones(1,numel(x_f))*track_f/2-i/2,z, 'k')
        ...
        if (app.DrawRearTiresCheckBox.Value)
            % RR
            plot3(app.UIAxes,x_r,ones(1,numel(x_r))*track_r/2+i/2,z, 'k')
            plot3(app.UIAxes,x_r,ones(1,numel(x_r))*track_r/2-i/2,z, 'k')
    ...
    if (app.DrawFrontSuspensionCheckBox.Value)
        % Plot fl lower A-arm
        plot3(app.UIAxes,[CHAS_LowForFront(1); UPRI_LowPntFront(1); CHAS_LowAftFront(1)],[CHAS_LowForFront(2); UPRI_LowPntFront(2); CHAS_LowAftFront(2)],[CHAS_LowForFront(3); UPRI_LowPntFront(3); CHAS_LowAftFront(3)],'Color',[0.9290 0.6940 0.1250]);
        plot3(app.UIAxes,[CHAS_LowForFront(1); UPRI_LowPntFront(1); CHAS_LowAftFront(1)],[-CHAS_LowForFront(2); -UPRI_LowPntFront(2); -CHAS_LowAftFront(2)],[CHAS_LowForFront(3); UPRI_LowPntFront(3); CHAS_LowAftFront(3)],'Color',[0.9290 0.6940 0.1250]);
        %scatter3(app.UIAxes,CHAS_LowFor(1),CHAS_LowFor(2),CHAS_LowFor(3)); % CHAS_LowFor
    
        % Plot fl upper A-arm
        plot3(app.UIAxes,[CHAS_UppForFront(1); UPRI_UppPntFront(1); CHAS_UppAftFront(1)],[CHAS_UppForFront(2); UPRI_UppPntFront(2); CHAS_UppAftFront(2)],[CHAS_UppForFront(3); UPRI_UppPntFront(3); CHAS_UppAftFront(3)],'Color',[0.9290 0.6940 0.1250]);
        plot3(app.UIAxes,[CHAS_UppForFront(1); UPRI_UppPntFront(1); CHAS_UppAftFront(1)],[-CHAS_UppForFront(2); -UPRI_UppPntFront(2); -CHAS_UppAftFront(2)],[CHAS_UppForFront(3); UPRI_UppPntFront(3); CHAS_UppAftFront(3)],'Color',[0.9290 0.6940 0.1250]);
    
        % Plot Tie rod
        plot3(app.UIAxes,[CHAS_TiePntFront(1); UPRI_TiePntFront(1)],[CHAS_TiePntFront(2); UPRI_TiePntFront(2)],[CHAS_TiePntFront(3); UPRI_TiePntFront(3)],'Color','blue');
        plot3(app.UIAxes,[CHAS_TiePntFront(1); UPRI_TiePntFront(1)],[-CHAS_TiePntFront(2); -UPRI_TiePntFront(2)],[CHAS_TiePntFront(3); UPRI_TiePntFront(3)],'Color','blue');   

        %scatter3(app.UIAxes,CHAS_TiePnt(1),CHAS_TiePnt(2),CHAS_TiePnt(3)); % CHAS_TiePnt
    end

    if (app.DrawFrontUprightCheckBox.Value)
        % Plot front uprights
        fill3(app.UIAxes,[UPRI_LowPntFront(1), UPRI_UppPntFront(1), UPRI_TiePntFront(1)],[UPRI_LowPntFront(2), UPRI_UppPntFront(2), UPRI_TiePntFront(2)],[UPRI_LowPntFront(3), UPRI_UppPntFront(3), UPRI_TiePntFront(3)],[0.9290 0.6940 0.1250]);
        fill3(app.UIAxes,[UPRI_LowPntFront(1), UPRI_UppPntFront(1), UPRI_TiePntFront(1)],[-UPRI_LowPntFront(2), -UPRI_UppPntFront(2), -UPRI_TiePntFront(2)],[UPRI_LowPntFront(3), UPRI_UppPntFront(3), UPRI_TiePntFront(3)],[0.9290 0.6940 0.1250]);
    end

    if (app.DrawRearSuspensionCheckBox.Value)
        % Plot fl lower A-arm
        plot3(app.UIAxes,[CHAS_LowForRear(1); UPRI_LowPntRear(1); CHAS_LowAftRear(1)],[CHAS_LowForRear(2); UPRI_LowPntRear(2); CHAS_LowAftRear(2)],[CHAS_LowForRear(3); UPRI_LowPntRear(3); CHAS_LowAftRear(3)],'Color',[0.9290 0.6940 0.1250]);
        plot3(app.UIAxes,[CHAS_LowForRear(1); UPRI_LowPntRear(1); CHAS_LowAftRear(1)],[-CHAS_LowForRear(2); -UPRI_LowPntRear(2); -CHAS_LowAftRear(2)],[CHAS_LowForRear(3); UPRI_LowPntRear(3); CHAS_LowAftRear(3)],'Color',[0.9290 0.6940 0.1250]);
        %scatter3(app.UIAxes,CHAS_LowFor(1),CHAS_LowFor(2),CHAS_LowFor(3)); % CHAS_LowFor
    
        % Plot fl upper A-arm
        plot3(app.UIAxes,[CHAS_UppForRear(1); UPRI_UppPntRear(1); CHAS_UppAftRear(1)],[CHAS_UppForRear(2); UPRI_UppPntRear(2); CHAS_UppAftRear(2)],[CHAS_UppForRear(3); UPRI_UppPntRear(3); CHAS_UppAftRear(3)],'Color',[0.9290 0.6940 0.1250]);
        plot3(app.UIAxes,[CHAS_UppForRear(1); UPRI_UppPntRear(1); CHAS_UppAftRear(1)],[-CHAS_UppForRear(2); -UPRI_UppPntRear(2); -CHAS_UppAftRear(2)],[CHAS_UppForRear(3); UPRI_UppPntRear(3); CHAS_UppAftRear(3)],'Color',[0.9290 0.6940 0.1250]);
    
        % Plot Tie rod
        plot3(app.UIAxes,[CHAS_TiePntRear(1); UPRI_TiePntRear(1)],[CHAS_TiePntRear(2); UPRI_TiePntRear(2)],[CHAS_TiePntRear(3); UPRI_TiePntRear(3)],'Color','blue');
        plot3(app.UIAxes,[CHAS_TiePntRear(1); UPRI_TiePntRear(1)],[-CHAS_TiePntRear(2); -UPRI_TiePntRear(2)],[CHAS_TiePntRear(3); UPRI_TiePntRear(3)],'Color','blue');
    ...
    if (app.DrawRearUprightCheckBox.Value)
        % Plot Rear uprights
        fill3(app.UIAxes,[UPRI_LowPntRear(1), UPRI_UppPntRear(1), UPRI_TiePntRear(1)],[UPRI_LowPntRear(2), UPRI_UppPntRear(2), UPRI_TiePntRear(2)],[UPRI_LowPntRear(3), UPRI_UppPntRear(3), UPRI_TiePntRear(3)],[0.9290 0.6940 0.1250]);
        fill3(app.UIAxes,[UPRI_LowPntRear(1), UPRI_UppPntRear(1), UPRI_TiePntRear(1)],[-UPRI_LowPntRear(2), -UPRI_UppPntRear(2), -UPRI_TiePntRear(2)],[UPRI_LowPntRear(3), UPRI_UppPntRear(3), UPRI_TiePntRear(3)],[0.9290 0.6940 0.1250]);
    end    
    ...
    if (app.DrawInstantCentersFrontCheckBox.Value)
        ...
        plot3(app.UIAxes,X_IC1_TireGround_f,Y_IC1_TireGround_f,Z_IC1_TireGround_f,'b-.')
        plot3(app.UIAxes,X_IC2_TireGround_f,Y_IC2_TireGround_f,Z_IC2_TireGround_f,'b-.')
        
        % Plot Instant Centers
        scatter3(app.UIAxes,x_fa,y_IC_f,z_IC_f,'filled','SizeData', 40, 'MarkerFaceColor', 'b');
        scatter3(app.UIAxes,x_fa,-y_IC_f,z_IC_f,'filled','SizeData', 40, 'MarkerFaceColor', 'b');
    end

    if (app.DrawInstantCentersRearCheckBox.Value)
        % Plot Lines between Instantaneous Center points and Tire-Ground Contact points 
        plot3(app.UIAxes,X_IC1_TireGround_r,Y_IC1_TireGround_r,Z_IC1_TireGround_r,'b-.')
        plot3(app.UIAxes,X_IC2_TireGround_r,Y_IC2_TireGround_r,Z_IC2_TireGround_r,'b-.')
        
        % Plot Instant Centers
        scatter3(app.UIAxes,x_ra,y_IC_r,z_IC_r,'filled','SizeData', 40, 'MarkerFaceColor', 'b');
        scatter3(app.UIAxes,x_ra,-y_IC_r,z_IC_r,'filled','SizeData', 40, 'MarkerFaceColor', 'b');
    end
    ...
    if (app.DrawFrontRollCenterCheckBox.Value)
        % Roll Center Front
        %scatter3(app.UIAxes,x_fa, 0, h_rc_f);
        scatter3(app.UIAxes,x_fa,round(y_RC_Front,5),z_RC_Front,'filled','SizeData', 40, 'MarkerFaceColor', '#0288d1');
    end

    if (app.DrawRearRollCenterCheckBox.Value)
        % Roll Center Rear
        %scatter3(app.UIAxes,x_fa+wheelbase, 0, h_rc_r);
        scatter3(app.UIAxes,x_ra,round(y_RC_Rear,5),z_RC_Rear,'filled','SizeData', 40, 'MarkerFaceColor', '#0288d1');
    end

    %% Roll axis
    if (app.DrawRollAxisCheckBox.Value)
        % Draw Roll axis
        plot3(app.UIAxes,[x_fa; x_fa+wheelbase],[0; 0],[z_RC_Front; z_RC_Rear], 'Color', '#0288d1')
    end
