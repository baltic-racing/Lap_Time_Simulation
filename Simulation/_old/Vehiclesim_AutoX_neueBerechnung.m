...
%% Noch zu bearbeiten
% - Schräglaufwiderstand als Fahrwiderstand einbauen
...
kappa_vl = zeros(1,length(Track));
kappa_vr = zeros(1,length(Track));
kappa_hl = zeros(1,length(Track));
kappa_hr = zeros(1,length(Track));
l_Latsch_vl = zeros(1,length(Track));
l_Latsch_vr = zeros(1,length(Track));
l_Latsch_hl = zeros(1,length(Track));
l_Latsch_hr = zeros(1,length(Track));
...
% 
% figure(2)
% scatter3(alpha_v(1:length(ApexIndizes)),FWYmax_vr(1:length(ApexIndizes)),...
%     FWZ_vr(1:length(ApexIndizes)),'filled')
% xlabel('Schräglaufwinkel [°]')
% ylabel('Max. Querkraft vorne rechts [N]')
% zlabel('Radlast vorne rechts [N]')
% grid minor
% 
% figure(3)
% scatter3(alpha_v(1:length(ApexIndizes)),FWYmax_vl(1:length(ApexIndizes)),...
%     FWZ_vl(1:length(ApexIndizes)),'filled')
% xlabel('Schräglaufwinkel [°]')
% ylabel('Max. Querkraft vorne links [N]')
% zlabel('Radlast vorne links [N]')
% grid minor

% figure(4)
% plot(1:65,FWZ_vl(1:length(ApexIndizes)),1:65,FWZ_vr(1:length(ApexIndizes)))
% legend('Radlast vorne links','Radlast vorne rechts')
% 
% figure(5)
% plot(1:65,FWZ_hl(1:length(ApexIndizes)),1:65,FWZ_hr(1:length(ApexIndizes)))
...
%     FT(i) = FVX(i)/2-k_R*FWZ_hl(i+1)-FB(i)/4;   % [N] Umfangskräfte an einem Hinterrad
%     l_Latsch_vl(i) = FWZ_vl(i)/(p_infl*bW);   % [m] Latschlänge vorne links
%     l_Latsch_vr(i) = FWZ_vr(i)/(p_infl*bW);   % [m] Latschlänge vorne rechts
%     l_Latsch_hl(i) = FWZ_hl(i)/(p_infl*bW);   % [m] Latschlänge hinten links
%     l_Latsch_hr(i) = FWZ_hr(i)/(p_infl*bW);   % [m] Latschlänge hinten rechts
%     
%     % Längsschlupf nach Carter
%     my0 = 2;    % [-] Haftreibungsbeiwert   
%     kappa_hl(i) = l_Latsch_hl(i)/(2*Rdyn_hl(i))*my0*(1-sqrt(1-FT(i)/(my0*FWZ_hl(i))));
%     kappa_hr(i) = l_Latsch_hr(i)/(2*Rdyn_hr(i))*my0*(1-sqrt(1-FT(i)/(my0*FWZ_hr(i))));
%     kappa_vl(i) = l_Latsch_vl(i)/(2*Rdyn_vl(i))*my0*(1-sqrt(1-FT(i)/(my0*FWZ_vl(i))));
%     kappa_vr(i) = l_Latsch_vr(i)/(2*Rdyn_vr(i))*my0*(1-sqrt(1-FT(i)/(my0*FWZ_vr(i))));
    
%     % Maximal übertragbare Reifenkräfte in Längsrichtung
%     FWXmax_vl(i+1) = max(abs(MF52_Fx_cs(alpha_v(i),FWZ_vl(i),GAMMA,0:0.01:0.2,TIRparam))); % [N] Maximal übertragbare Radkraft
%     FWXmax_vr(i+1) = abs(MF52_Fx_cs(alpha_v(i),FWZ_vr(i),GAMMA,kappa_vr(i),TIRparam)); % [N] Maximal übertragbare Radkraft
%     FWXmax_hl(i+1) = abs(MF52_Fx_cs(alpha_h(i),FWZ_hl(i),GAMMA,kappa_hl(i),TIRparam)); % [N] Maximal übertragbare Radkraft
%     FWXmax_hr(i+1) = abs(MF52_Fx_cs(alpha_h(i),FWZ_hr(i),GAMMA,kappa_hr(i),TIRparam)); % [N] Maximal übertragbare Radkraft
...
%     FWXmax_h(i+1) = FWXmax_hl(i+1) + FWXmax_hr(i+1);    % [N] Maximal übertragbare Achskraft
        ...
        % Maximal übertragbare Reifenkräfte in Längsrichtung
    FWXmax_vl(i+1) = max(abs(MF52_Fx_cs(alpha_v(i),FWZ_vl(i),GAMMA,0:0.01:0.2,TIRparam))); % [N] Maximal übertragbare Radkraft
    FWXmax_vr(i+1) = max(abs(MF52_Fx_cs(alpha_v(i),FWZ_vr(i),GAMMA,0:0.01:0.2,TIRparam))); % [N] Maximal übertragbare Radkraft
    FWXmax_hl(i+1) = max(abs(MF52_Fx_cs(alpha_h(i),FWZ_hl(i),GAMMA,0:0.01:0.2,TIRparam))); % [N] Maximal übertragbare Radkraft
    FWXmax_hr(i+1) = max(abs(MF52_Fx_cs(alpha_h(i),FWZ_hr(i),GAMMA,0:0.01:0.2,TIRparam))); % [N] Maximal übertragbare Radkraft
    FWXmax_v(i+1) = FWXmax_vl(i+1) + FWXmax_vr(i+1);    % [N] Maximal übertragbare Achskraft
    FWXmax_h(i+1) = FWXmax_hl(i+1) + FWXmax_hr(i+1);    % [N] Maximal übertragbare Achskraft
...
% figure(1)
% plot(s,FWXmax_hl,s,FWXmax_hl,s(1:end-1),FVX)
% legend('hl','hr','Zugkraft')
% 
% figure(2)
% plot(s,FWZ_hr,s,FWZ_hl)
% legend('hr','hl')
% 
% figure(3)
% plot(s,FWZ_vr,s,FWZ_vl)

vV_NoBrake = vV;    % Zwischenspeichern von Geschwindigkeitsverlauf für Plot 
Gangauswahl_NoBrake = Gangauswahl;
ApexGang_Soll = Gangauswahl(ApexIndizes);
speicherhl = FWXmax_hl;
speicherhr = FWXmax_hr;
speicherFWZhl = FWZ_hl;
speicherFWZhr = FWZ_hr;
speicherFWZvl = FWZ_vl;
speicherFWZvr = FWZ_vr;
KAPPAHL = kappa_hl;
...
for i = 1:length(Track)-1
    ...
    if ismember(i,ApexIndizes)  % Überprüfen, vor welcher Apex das Auto ist
        z = z + 1;
    end
    
    if R(i) > 0     % Hilfsgröße für Schrägwinkelberechnung
        f = -1;
    else
        f = 1;
    end
    ...
    % Maximal übertragbare Reifenkräfte in Längsrichtung
%     FWXmax_vl(i+1) = max(MF52_Fx_cs(0,FWZ_vl(i+1),GAMMA,-0.2:0.01:0.2,TIRparam)); % [N] Maximal übertragbare Radkraft
%     FWXmax_vr(i+1) = max(MF52_Fx_cs(0,FWZ_vr(i+1),GAMMA,-0.2:0.01:0.2,TIRparam)); % [N] Maximal übertragbare Radkraft
%     FWXmax_hl(i+1) = max(MF52_Fx_cs(0,FWZ_hl(i+1),GAMMA,-0.2:0.01:0.2,TIRparam)); % [N] Maximal übertragbare Radkraft
%     FWXmax_hr(i+1) = max(MF52_Fx_cs(0,FWZ_hr(i+1),GAMMA,-0.2:0.01:0.2,TIRparam)); % [N] Maximal übertragbare Radkraft
%     FWXmax_v(i+1) = FWXmax_vl(i+1) + FWXmax_vr(i+1);    % [N] Maximal übertragbare Achskraft
%     FWXmax_h(i+1) = FWXmax_hl(i+1) + FWXmax_hr(i+1);    % [N] Maximal übertragbare Achskraft
...
%     FWXmax_vl(i+1) = abs(MF52_Fx_cs(alpha_v(i),FWZ_vl(i+1),GAMMA,kappa_vl(i),TIRparam)); % [N] Maximal übertragbare Radkraft
%     FWXmax_vr(i+1) = abs(MF52_Fx_cs(alpha_v(i),FWZ_vr(i+1),GAMMA,kappa_vr(i),TIRparam)); % [N] Maximal übertragbare Radkraft
%     FWXmax_hl(i+1) = abs(MF52_Fx_cs(alpha_h(i),FWZ_hl(i+1),GAMMA,kappa_hl(i),TIRparam)); % [N] Maximal übertragbare Radkraft
%     FWXmax_hr(i+1) = abs(MF52_Fx_cs(alpha_h(i),FWZ_hr(i+1),GAMMA,kappa_hr(i),TIRparam)); % [N] Maximal übertragbare Radkraft
%     FWXmax_v(i+1) = FWXmax_vl(i+1) + FWXmax_vr(i+1);    % [N] Maximal übertragbare Achskraft
%     FWXmax_h(i+1) = FWXmax_hl(i+1) + FWXmax_hr(i+1);    % [N] Maximal übertragbare Achskraft
...
%     % Schräglaufseitensteifigkeiten
%     cY_vl(i+1) = interp1(Fzy,cY_tire,FWZ_vl(i+1),'linear','extrap'); % [N/°] Schräglaufseitensteifigkeit interpoliert
%     cY_vr(i+1) = interp1(Fzy,cY_tire,FWZ_vr(i+1),'linear','extrap'); % [N/°] Schräglaufseitensteifigkeit interpoliert
%     cY_hl(i+1) = interp1(Fzy,cY_tire,FWZ_hl(i+1),'linear','extrap'); % [N/°] Schräglaufseitensteifigkeit interpoliert
%     cY_hr(i+1) = interp1(Fzy,cY_tire,FWZ_hr(i+1),'linear','extrap'); % [N/°] Schräglaufseitensteifigkeit interpoliert
%     
%     % Schräglaufwinkel
%     alpha_vl(i+1) = FWYv(i)/2/cY_vl(i+1);  % [°] Schräglaufwinkel vorne links
%     alpha_vr(i+1) = FWYv(i)/2/cY_vr(i+1);  % [°] Schräglaufwinkel vorne rechts
%     alpha_hl(i+1) = FWYh(i)/2/cY_hl(i+1);  % [°] Schräglaufwinkel hinten links
%     alpha_hr(i+1) = FWYh(i)/2/cY_hr(i+1);  % [°] Schräglaufwinkel hinten rechts
    
%     % Gierbeschleunigung und -geschwindigkeit
%     psi2(i) = (FWYv(i)*lv - FWYh(i)*lh)/thetaV_Z;  % [rad/s²]  
%     psi1(i+1) = psi1(i) + psi2(i)*(t(i+1)-t(i));   % [rad/s] 
%     psi1neu(i) = vV(i)/R(i); % Gierrate um Kurvenmittelpunkt
