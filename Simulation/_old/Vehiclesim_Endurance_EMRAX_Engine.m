...
%% Matfiles einlesen
%load('EMRAX_208_TorqueData_Continuous.mat');  % Momentenverlauf EMRAX 
load('EMRAX_208_TorqueData_Peak.mat');  % Momentenverlauf EMRAX
load('Aero Downforce Daten.mat');             % Aero Daten
load('EnduranceTrack.mat');                   % Streckendaten
 
x_Track = Track(:,1);           % [m] X-Koordinate der Strecke
y_Track = Track(:,2);           % [m] Y-Koordinate der Strecke
z_Track = Track(:,3);           % [m] Z-Koordinate der Strecke
s = Track(:,4);                 % [m] Verlauf der Streckenlänge
R = Track(:,5);                 % [m] Kurvenradien
...
m_ph = 50;           % [%] Prozentualer Radlastanteil Hinterachse
...
t_L = 35;       % [°C] Umgebungslufttemperatur
...
n_Mmax = 6000;      % [1/min] Maximaldrehzahl des Motors    % [-] Gesamtwirkungsgrad des Antriebsstrangs
max_power = 40000 ; % Power Limit in endurance in W

% Getriebedaten
z_Kettenblatt = 66;     % [-] Zähnezahl des Kettenblatts
z_Ritzel = 17;          % [-] Zähnezahl des Ritzels
i_G = z_Kettenblatt / z_Ritzel; % [-] Übersetzung Motor zu Rad

% Akkudaten
V_i = 550; % [V]
Energy_i = 7.5; %[kWh]
nZellen_parralel = 5;
Kapazitaet_einzelzelle = 10.8; %[Wh]
anzahl_zellen_in_reihe = 131;
Kapazitaet_gesamtakku = anzahl_zellen_in_reihe*nZellen_parralel*Kapazitaet_einzelzelle; %[Wh]
...
TIRparam.LMUX = 0.65;          % [-] Skalierungsfaktor Längsrichtung
TIRparam.LMUY = 0.65;          % [-] Skalierungsfaktor Querrichtung
...
for i = 1:length(ApexIndizes)
    ...
    while  FWYv(i) < FWYmax_v(i) && FWYh(i) < FWYmax_h(i) && vV(i) < 30
        ...
        FWYmax_vl(i) = max(abs(MF52_Fy_cs(0:0.1:12,FWZ_vl(i),GAMMA,0,TIRparam))); % [N] Maximal übertragbare Radkraft
        FWYmax_vr(i) = max(abs(MF52_Fy_cs(0:0.1:12,FWZ_vr(i),GAMMA,0,TIRparam))); % [N] Maximal übertragbare Radkraft
        FWYmax_hl(i) = max(abs(MF52_Fy_cs(0:0.1:12,FWZ_hl(i),GAMMA,0,TIRparam))); % [N] Maximal übertragbare Radkraft
        FWYmax_hr(i) = max(abs(MF52_Fy_cs(0:0.1:12,FWZ_hr(i),GAMMA,0,TIRparam))); % [N] Maximal übertragbare Radkraft
...
%% Startwerte für ersten Simulationslauf OHNE BREMSEN
vV(1) = 15;    % [m/s] Geschwindigkeit
t(1) = 0;      % [s] Zeit

% Hilfsgrößen
z = 1;         % [-] Bestimmung der anstehenden Apex

%% Simulation OHNE BREMSEN
NonBrakeApexes = [];

for i = 1:length(Track)-1
    
    % Bestimmen von Motordrehzahl und Gang
    ni(i) = vV(i)*30/pi*i_G/Rdyn_hl(i); % [1/min] Aktuelle Motordrehzahl ermitteln
    ...
    drehzahlpointer = round(ni(i)); %Pointer für effizienztabelle
    Momentenpointer = round(Mi(i)); %Pointer für effizienztabelle
   
    P_M(i) = (2*Mi(i)*ni(i)/60*2*pi);      % [W] Gesamt-Motorleistung
    if P_M(i) > max_power
        P_M(i) = max_power;                  % [W] Begrenzte Leistung
        Mi(i) = P_M(i)*60/ni(i)/2/pi/2;  % [Nm] Begrenzen des Moments     
    end
    
    P_M(i) = P_M(i)*M_eff_inter(drehzahlpointer,Momentenpointer); %berechnung der Motorleistung nach abzug der Effizienz
    
    % Berechnen der Zugkraft
    FVX_hl(i) = Mi(i)*i_G/Rdyn_hl(i); % [N] Zugkraft an linkem Hinterrad
    FVX_hr(i) = Mi(i)*i_G/Rdyn_hr(i); % [N] Zugkraft an rechtem Hinterrad
    ...
    FWXmax_vl(i+1) = max(abs(MF52_Fx_cs(0,FWZ_vl(i+1),GAMMA,0:0.01:0.2,TIRparam))); % [N] Maximal übertragbare Radkraft
    FWXmax_vr(i+1) = max(abs(MF52_Fx_cs(0,FWZ_vr(i+1),GAMMA,0:0.01:0.2,TIRparam))); % [N] Maximal übertragbare Radkraft
    FWXmax_hl(i+1) = max(abs(MF52_Fx_cs(0,FWZ_hl(i+1),GAMMA,0:0.01:0.2,TIRparam))); % [N] Maximal übertragbare Radkraft
    FWXmax_hr(i+1) = max(abs(MF52_Fx_cs(0,FWZ_hr(i+1),GAMMA,0:0.01:0.2,TIRparam))); % [N] Maximal übertragbare Radkraft
    ...
    % Maximal übertragbare Reifenkräfte in Querrichtung
    FWYmax_vl(i+1) = max(abs(MF52_Fy_cs(0:0.1:12,FWZ_vl(i),GAMMA,0,TIRparam))); % [N] Maximal übertragbare Radkraft
    FWYmax_vr(i+1) = max(abs(MF52_Fy_cs(0:0.1:12,FWZ_vr(i),GAMMA,0,TIRparam))); % [N] Maximal übertragbare Radkraft
    FWYmax_hl(i+1) = max(abs(MF52_Fy_cs(0:0.1:12,FWZ_hl(i),GAMMA,0,TIRparam))); % [N] Maximal übertragbare Radkraft
    FWYmax_hr(i+1) = max(abs(MF52_Fy_cs(0:0.1:12,FWZ_hr(i),GAMMA,0,TIRparam))); % [N] Maximal übertragbare Radkraft
...
E_Akku(1) = 0;       % [J] Verbrauchte Energie Akku
E_Waerme(1) = 0; 
E_Akku_Reku(i) = 0;  % [J] Rekuperierte Energie Akku
...
for i = 1:length(Track)-1
    ...
    % Aero-Abtriebsraft interpoliert
    Faero(i) = interp1(v,FA,vV(i)*3.6,'linear','extrap'); % [N]
    
    % Prüfen, ob gebremst werden muss
    if ismember(i,BrakeIndizes) % Einleiten des Bremsvorgangs     
        Mi(i) = 0;          % [Nm] Motormoment
        BPPsignal(i) = 1;   % [-] Bremssignal
        P_Bh(i) = FWZh(i)/FWZges(i)*FB(i)*vV(i);    % [W] Bremsleistung hinten für Rekuperation
    else
        Mi(i) = interp1(n,M,ni(i),'linear','extrap'); % [Nm] Motormoment
        FB(i) = 0;          % [N] Bremskraft
        P_Bh(i) = 0;        % [W] Bremsleistung hinten
    end
...
% Motorleistung & Begrenzung auf 80 kW aus FS-Rules
    ...
    A_Akkuzelle(i) = P_M(i) / V_i / nZellen_parralel;
    
    P_M(i) = P_M(i)*M_eff_inter(drehzahlpointer,Momentenpointer); %berechnung der Motorleistung nach abzug der Effizienz  
    %Mi(i) = P_M(i)/2*pi*ni(i); some issue dont understand
    P_Mloss(i) = P_M(i)*(1-M_eff_inter(drehzahlpointer,Momentenpointer));%berechnung der Verlustleistung
    ...
    if ismember(i,BrakeIndizes) && vV(i+1) < vAPEXmax(z) && not(ismember(z,NonBrakeApexes))  % Begrenzen der Geschwindigkeit auf ApexGeschwindigkeit
    ...
    E_Akku(i+1) = E_Akku(i) + (t(i+1)-t(i)) * P_M(i);   % [J] 
    E_Waerme(i+1) = P_M(i)*(1-M_eff_inter(drehzahlpointer,Momentenpointer))+(E_Akku(i) + (t(i+1)-t(i)))*0.05;   % [J] Motorverluste + 5% flat für inverterverlsute
...
% Umrechnen der Energiemengen des Akkus
E_Akku = E_Akku/(3.6e6);  % [kWh] Verbrauchte Akku-Energie je Runde
E_Waerme = E_Waerme/(3.6e6);
E_Akku_Reku = E_Akku_Reku/(3.6e6);    % [kWh] Rekuperierte Akku-Energie je Runde
E_ges = E_Akku - E_Akku_Reku;         % [kWh] Resultierender Verbrauch je Runde
...
% title('Endurance-Strecke Formula Student Germany','FontSize',12) 
% for z = 1:24
...
% Geschwindigkeit über Strecke mit und ohne Bremsen
figure(2)
...
% Geschwindigkeitsverlauf auf der Strecke
figure(3)
...
% Leistungsverlauf auf Strecke
figure(9)
scatter(Track(1:end-1,1),Track(1:end-1,2),40,P_M/1000,'filled')
title('Leistungsverlauf','FontSize',12) 
xlabel('X-Koordinate [m]','FontSize',10)
ylabel('Y-Koordinate [m]','FontSize',10)
colormap(jet)
c = colorbar;
ylabel(c,'Gesamt-Motorleistung [kW]')
box on
axis equal

% Verbrauchte Akku-Energie über Strecke
figure(10)
plot(s,E_Akku,s,E_Akku_Reku,s,E_Akku-E_Akku_Reku,s,E_Waerme)
title('Akku-Energie','FontSize',12) 
legend('Verbraucht','Rekuperiert','Resultierend','Waerme')
xlabel('Strecke [m]','FontSize',10) 
ylabel('Energie [kWh]','FontSize',10)
annotation('textbox', [0.2, 0.7, 0.2, 0.1],...
    'String', {['Verbrauchte Energie = ' num2str(round(E_Akku(end),2)) ' kWh'],...
    ['Rekuperierte Energie = ' num2str(round(E_Akku_Reku(end),2)) ' kWh'],...
    ['Waerme Energie = ' num2str(round(E_Waerme(end),4)) ' kWh'],...
    ['Resultierender Verbrauch = ' num2str(round(E_ges(end),2)) ' kWh']})
grid on
box on
...
% Strombelastung einzelne Akkuzelle

figure(90)
plot(s,A_Akkuzelle)
title('Stromfluss','FontSize',12) 
legend('Zellenstrom')
xlabel('Strecke [m]','FontSize',10) 
ylabel('Strom [A]','FontSize',10)
grid on
box on


%%  Ausgabe der Werte
disp(['Endurance-Zeit für EINE Runde: ' num2str(t(end)) ' s']);
t_ges = t(end) *(22000/s(end));
disp(['Endurance-Zeit gesamt: ' num2str(t_ges(end)) ' s']);
disp(['Total Travel Distance: ' num2str(s(end)) ' m']);
E_Akku_gesamt_ohne_reku = E_Akku(end) * (22000/s(end));
disp(['Gesamtenergieverbrauch ohne Reku: ' num2str(E_Akku_gesamt_ohne_reku) ' kWh']);
E_Akku_gesamt = E_ges(end) * (22000/s(end));
disp(['Gesamtenergieverbrauch mit Reku: ' num2str(E_Akku_gesamt) ' kWh']);
toc